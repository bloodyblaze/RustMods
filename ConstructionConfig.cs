...
{
    [Info("ConstructionConfig", "Nogrod", "1.0.2", ResourceId = 859)]
    class ConstructionConfig : RustPlugin
    {
        ...
        private void CheckConfig()
        {
            ...
            Puts("Incorrect config version(" + Config["Version"] + ") move to .old");
        ...
        private void UpdateConstructions()
        {
            ...
            var protectionProperties = new HashSet<ProtectionProperties>();
            var constructionPrefabs = PrefabAttribute.server.GetAll<Construction>();
            var itemsDict = SingletonComponent<ItemManager>.Instance.itemList.ToDictionary(i => i.itemid);
            foreach (var common in constructionPrefabs)
            {
                if (constructions[common.hierachyName] == null)
                {
                    Puts(common.hierachyName + " doesn't exist in config");
                    continue;
                }
                ...
                for (var g = 0; g < common.grades.Length; g++)
                {
                    ...
                    {
                        Puts("Can't create grade: " + gradeType + " for: " + common.hierachyName);
                        continue;
                    }
                    ...
                    foreach (var cost in costToBuild)
                    {
                        ...
                        itemsDict.TryGetValue(itemid, out definition);
